...
#                Get-ImcLocalUser  -AccountStatus "active" | Set-ImcLocalUser -Pwd $ImpPass -Force
        ...
        If ($Host_Updated) {
            Write-Host "System $( $ChassisInfo.Serial ) was configured and rebooted via CIMC tools.  PXE installation is underway."
            Add-Content vDCM_BIOS_Boot.log "System $( $ChassisInfo.Serial ) was configured and rebooted via CIMC tools.  PXE installation is underway."
            $Host_Updated = 0
            }
        Else {
